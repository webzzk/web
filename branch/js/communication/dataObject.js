...
function Paper(){
    ...
    this.isUpdate=false;                            //scoresStr是否已更新
}
...
Paper.prototype.sumRate=function(){
    ...
    this.scoreMap.eachMap(function(key,value){
        if(value.isRight){
            ...
            score+=parseInt(value.rate);
            /*            if(isNum(value.rate)||value.rate==-5){
             score+=Number(value.rate);
             }
             else{
             //score+=Number(value.rate);
             score+=Number((value.rate/sumRate)*totalScore).toFixed(1);
             console.log(score);
             }*/
            //console.log(value.rate+"===="+parseFloat((value.rate/sumRate)*totalScore).toFixed(1))
            ...
            if(paper.scoreMap.find(this.actionMap.get(handleID).SID)){
                if(!preIsAction){
                    ...
                    if(paperFlag){
                        ...
                        console.log(paper.scoreMap.get(this.actionMap.get(handleID).SID))
...
HandleArray.prototype.setCondition=function(id,value){
    ...
    console.log(valueObject.find(id));
    if(valueObject.find(id)){
        valueObject.get(id).value=value;
        console.log(valueObject.get(id));
        valueObject.get(id).cdtMap.eachMap(function(key,value){
            ...
            var flag=false;
            //console.log("value.isAction"+value.isAction);
            if(value.isAction){
